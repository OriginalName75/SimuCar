	...
	public float createCars(float t, float deltaT, Map m, boolean fake) {
		...
		if (deltaT >= 1.0f && t < 1.0f) {
			if (!fake) {
				...
				addCarRoad(m.getRoads().get(0), m, "voiture 1", 11.111f, it);
				addCarRoad(m.getRoads().get(4), m, "voiture 2", 11.111f, it3);
			...
			if (!fake) {
				addCarRoad(m.getRoads().get(4), m, "voiture 3", 11.111f, it4);
